...
class PluginTests(unittest.TestCase):
    def test_applies_discovered_mutator(self) -> None:
        ...
        with patch("dockly.plugins._iter_entry_points", return_value=[_EntryPoint("append-label", _AppendingPlugin)]):
    ...
    def test_isolates_plugin_failures(self) -> None:
        ...
        with patch("dockly.plugins._iter_entry_points", return_value=[_EntryPoint("broken", _FailingPlugin)]):
            result = apply_dockerfile_mutators("FROM base\n", options)
    ...
    def test_detect_build_tool_from_plugins_returns_single_value(self) -> None:
        with patch(
            "dockly.plugins._iter_entry_points",
    ...
    def test_recipe_renderer_plugin(self) -> None:
        ...
        with patch("dockly.plugins._iter_entry_points", return_value=[_EntryPoint("acme", _recipe_renderer)]):
    ...
    def test_verify_renderer_plugin(self) -> None:
        ...
        with patch("dockly.plugins._iter_entry_points", return_value=[_EntryPoint("acme-json", _render_verify)]):
    ...
    def test_register_command_plugins_returns_warnings(self) -> None:
        with patch("dockly.plugins._iter_entry_points", return_value=[_EntryPoint("ok", _RegisterCommand)]):
