...
`export-config` writes `.dockly.toml` from plugin options so teams can later adopt the Python CLI. Re-export overwrites only with `-Dspringdocker.force=true`. Plugin configuration remains SSOT for the builder path.
