...
from dockly.gradle_descriptors import DEFAULT_GRADLE_DESCRIPTOR_FILES
from dockly.java_features import MIN_JAVA_VERSION, validate_dockerfile_options
from dockly.runtime_images import SUPPORTED_RUNTIME_IMAGES
...
DOCKLY_REPO_URL = "https://github.com/mnafshin/dockly"
...
NATIVE_AOT_DOCKERFILE_SCAFFOLD_COMMENT = (
    "# scaffold: experimental native-image Dockerfile; not a production-ready dockly workflow"
)
...
class DockerfileDocument:
    ...
    def render(self) -> str:
        ...
        env = Environment(
            loader=PackageLoader("dockly", "templates"),
            autoescape=False,
            trim_blocks=False,
            lstrip_blocks=False,
...
def _compose_dockerfile(spec: DockerfileSpec) -> DockerfileDocument:
    ...
    header_lines = [
        ...
        f"# Generated by dockly — {DOCKLY_REPO_URL}",
        ...
        _section(
            ...
            *_layered_appcds_training_run_lines()
            ...
            (
                ...
                "printf '{\"spdxVersion\":\"SPDX-2.3\",\"name\":\"dockly-generated-image\"}' > /tmp/sbom/spdx.json"
            )
            if spec.supply_chain.include_embedded_sbom
            else "",
            "",
