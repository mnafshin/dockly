...
## `dockly doctor` reports project detection failures
...
- Run `dockly inspect --project-root <path> --format json` to see detected metadata.
...
- `dockly inspect` recommends `--project-root` when it finds a single Spring Boot module under a reactor.
- Exotic layouts (dynamic Gradle includes, Bazel, mixed tooling) need a `dockly.project_detectors` plugin — examples in [`docs/examples/extensions/`](examples/extensions/).

## `dockly dockerfile generate` fails

- Run `dockly init --project-root <path>` first to create `.dockly.toml`.
